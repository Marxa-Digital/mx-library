...
export class MxAuth {
  ...
  notFoundMessage: string = 'Usuario no encontrado'
  invalidMessage: string = 'Usa una dirección de email correcta'
  wrongPasswordMessage: string = 'Contraseña Incorrecta'
  ...
  private async updateUserData( { uid, email, displayName, photoURL }: firebase.User ) {
    ...
    const userRef: AngularFirestoreDocument<firebase.User> = this.afs.doc( `${this.userCollection}/${ uid }` );
  ...
  restorePwd(email: string, message?: string) {
    var messageSent = !message ? `Se envió un email a ${email} para restaurar la contraseña` : message
