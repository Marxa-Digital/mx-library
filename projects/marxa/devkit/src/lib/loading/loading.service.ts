...
export class MxLoading {
  ...
  public loadingBox?: MatDialogRef<MxLoadingOverlayComponent> | null;
  private afterSubs?: Subscription | null;
  toggleWaiting(open: 'open' | 'close' | boolean) {
    open = open ? open : !open;
    if (open === true || open === 'open') {
      if (!this.loadingBox || this.loadingBox.getState() === 1) {
        this.loadingBox = this._dialog.open(MxLoadingOverlayComponent, {
          panelClass: 'loading-spinner',
        });
      } else {
        this.loadingBox.close();
        this.afterSubs = this.loadingBox.afterClosed().subscribe(() => {
          this._dialog.open(MxLoadingOverlayComponent, {
            panelClass: 'loading-spinner',
            id: this.loadingBox?.id,
          });
        });
      }
