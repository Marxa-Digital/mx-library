...
export class MatCarouselComponent
  ...
  @Input() public svgIconOverrides: SvgIconOverrides = {} as SvgIconOverrides;
  ...
  > = {} as QueryList<MatCarouselSlideComponent> ;
  ...
  public listKeyManager: ListKeyManager<MatCarouselSlideComponent>
    = {} as ListKeyManager<MatCarouselSlideComponent>;
  ...
  private timer$: Observable<number> = {} as Observable<number>;
  ...
  public ngAfterViewInit(): void {
    ...
    this.slides$
      .pipe(
        ...
        filter(value => value && value < this.slidesList.length ? true : false)
      )
  ...
  private isOutOfBounds(): boolean {
    const sign = this.orientation === 'rtl' ? -1 : 1;
      const nativeLeft = this.carouselList.nativeElement.getBoundingClientRect().left
      var offsetParentLeft = 0
      if (this.carouselList.nativeElement.offsetParent) {

          offsetParentLeft = this.carouselList
              .nativeElement
              .offsetParent
    ...
    const left =
      sign * (nativeLeft -( offsetParentLeft ? offsetParentLeft : 0));
  ...
  private getOffset(): number {
    const activeItems = this.listKeyManager.activeItemIndex
    const offset = (activeItems ? activeItems : 0) * this.getWidth();
    const sign = this.orientation === 'rtl' ? 1 : -1;
    return sign * offset;
  }
  ...
  private playAnimation(): void {
    ...
    animation.onDone(() => {
      this.change.emit(this.currentIndex?this.currentIndex:0);
      this.playing = false;
      this.renderer.setStyle(
        this.carouselList.nativeElement,
        'transform',
